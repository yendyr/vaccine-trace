...
class JournalProcess
{
    ...
    public static function stockInboundAutoJournal(StockMutation $stockMutationRow) {
        ...
        // Check and Destroy Existing Related Journal with this Stock Inbound
        if ($stockMutationRow->journal) {
    ...
    public static function stockOutboundAutoJournal(StockMutation $stockMutationRow) {
        ...
        // Check and Destroy Existing Related Journal with this Stock Outbound
        if ($stockMutationRow->journal) {
            foreach ($stockMutationRow->journal->journal_details as $journal_detail) {
                $journal_detail->delete();
            }
        ...
        foreach ($target_items as $target_item) {
            ...
            if (!JournalDetail::where('journal_id', '=', $Journal->id)->where('coa_id', '=', $cost_coa_id)->exists()) {
                JournalDetail::create([
                    'uuid' =>  Str::uuid(),
                ...
                JournalDetail::where('journal_id', '=', $Journal->id)->where('coa_id', '=', $cost_coa_id)->increment(
