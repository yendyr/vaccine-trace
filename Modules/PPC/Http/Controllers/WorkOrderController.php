...
class WorkOrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!$work_order->id) {
            ...
            $code = 'WKORD-' .  $transaction_date->year . '-' . str_pad(dechex($work_order->id), 9, '0', STR_PAD_LEFT);
        ...
        if (sizeof($jobcards) > 0) {
            foreach ($jobcards as $jobcard_row) {
                ...
                $new_jc_code = $jobcard_row->update([
                    'code' => 'JBCRD-' .  $transaction_date->year . '-' . str_pad(dechex($jobcard_row->id), 9, '0', STR_PAD_LEFT)
                ]);
