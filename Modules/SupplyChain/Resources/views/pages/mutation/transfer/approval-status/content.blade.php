<div class="row m-b fadeIn animate-md">
            ...
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-10">
                        ...
                        <p><strong>{{ $MutationTransfer->approvals->first()->creator->name ?? 'Not Yet Approved' }}</strong></p>
                        
                        <p class="m-b-none">Approved Date:</p>
                        <p><strong>{{ $MutationTransfer->approvals->first()->created_at ?? 'Not Yet Approved' }}</strong></p>
                    </div>
    ...
    @if($MutationTransfer->approvals()->count() == 0)
        @can('approval', Modules\SupplyChain\Entities\StockMutation::class)
            @component('components.approve-modal', ['name' => 'Mutation Transfer Datalist'])
            @endcomponent

            <div class="col-md-4">
                <button type="button" class="approveBtn btn btn-lg btn-outline btn-success" data-toggle="tooltip" title="Approve"
                    value="{{ $MutationTransfer->id }}">
                    <i class="fa fa-check-circle"></i>&nbsp;Approve Now
                </button>
            </div>

            @include('supplychain::components.mutation.transfer.approval-status._script')
        @endcan
    @endif
</div>